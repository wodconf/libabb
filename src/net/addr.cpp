...
bool SockAddr::SetV4(const char *addr, uint16_t port){
	int res = 1;
...
bool SockAddr::SetV6(const char *addr, uint16_t port){
	int res = 1;
...
bool SockAddr::SetUnix(const char *path,unsigned int pathlen){
	if (!path) {
		return false;
	}
	if (*path == 0) {
...
bool SockAddr::SetFromAddrInfo(struct addrinfo* ai){
...
bool SockAddr::SetFromHostent(struct hostent *ent)
{
